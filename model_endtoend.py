...
class EndToEndNeuralNetworkModel(nn.Module):
    def __init__(self, input_dim, hidden_dims, fc_dims, extra_feature_dim):
        super(EndToEndNeuralNetworkModel, self).__init__()
        ...
        self.fc_layers = []
        prev_dim = 2 * (input_dim + sum(hidden_dims)) + extra_feature_dim                   
    ...
    def forward(self, inputs_1, inputs_2, extra_features, output_dist = False):
        ...
        for i in range(self.nb_encoder_layers + 1):
            feature_vectors.append(outs_1[i])
            feature_vectors.append(outs_2[i])
